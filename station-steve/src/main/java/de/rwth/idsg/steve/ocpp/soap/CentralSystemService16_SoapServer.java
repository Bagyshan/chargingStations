...
public class CentralSystemService16_SoapServer implements CentralSystemService {
    ...
    public BootNotificationResponse bootNotificationWithTransport(BootNotificationRequest parameters,
        ...
        BootNotificationResponse response = service.bootNotification(parameters, chargeBoxIdentity, protocol);

        StationCreateEvent event = new StationCreateEvent(
                chargeBoxIdentity,
                protocol.getVersion().getValue(),
        ...
        stationEventProducer.sendStationCreated(event);
    ...
    public StatusNotificationResponse statusNotification(StatusNotificationRequest parameters,
                                                         String chargeBoxIdentity) {
        boolean connector = this.ocppServerRepository.hasConnector(parameters.getConnectorId(), chargeBoxIdentity);
        StatusNotificationResponse response = service.statusNotification(parameters, chargeBoxIdentity);

        if (!connector) {
            ConnectorCreateEvent event = new ConnectorCreateEvent(
                    chargeBoxIdentity,
