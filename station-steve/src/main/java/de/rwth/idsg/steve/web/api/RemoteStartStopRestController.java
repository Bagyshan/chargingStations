...
import de.rwth.idsg.steve.web.api.dto.RemoteStartResponse;
import de.rwth.idsg.steve.web.api.dto.RemoteStopResponse;
...
//    @Autowired private ApiTransactionService apiTransactionService;
    ...
    @ResponseBody
    public RemoteStartResponse postRemoteStartTx(@Valid ApiChargePointStart params) {
        ...
        client.remoteStartTransaction(transactionParams, "SteveWebApi");

        final long POLL_INTERVAL_MS = 200L;   // интервал между попытками
        final long TIMEOUT_MS = 5_000L;      // общий таймаут ожидания транзакции

        long waitStart = System.currentTimeMillis();
        Integer newTransactionId = null;

        while (System.currentTimeMillis() - waitStart < TIMEOUT_MS) {
            newTransactionId = transactionRepository.getActiveTransactionId(params.getChargeBoxId(), params.getConnectorId());
            if (newTransactionId != null) {
                break; // нашли запись — выходим
            }
            try {
                Thread.sleep(POLL_INTERVAL_MS);
            } catch (InterruptedException ie) {
                // Восстанавливаем статус прерывания и выходим — чтобы не игнорировать внешний interrupt
                Thread.currentThread().interrupt();
                throw new IllegalStateException("Thread interrupted while waiting for transaction creation", ie);
            }
        }

        if (newTransactionId == null) {
            // Выбор поведения: бросать ошибку, или вернуть ответ без данных транзакции.
            // Здесь делаем жёсткий выбор — бросаем исключение, чтобы caller знал о проблеме.
            throw new IllegalStateException(String.format(
                    "Transaction was not created within %d ms after remoteStart for chargeBoxId=%s connectorId=%d",
                    TIMEOUT_MS, params.getChargeBoxId(), params.getConnectorId()
            ));
        }

        Transaction transaction = transactionRepository.getTransaction(newTransactionId);

        return new RemoteStartResponse(
                transaction.getId(),
                params.getChargeBoxId(),
                params.getConnectorId(),
                params.getOcppTag(),
                transaction.getStartValue(),
    ...
    @ResponseBody
    public RemoteStopResponse postRemoteStopTx(@Valid ApiChargePointStart params) {
        // only one active task per charge box over api; to be discussed!
        if (activeTaskOnChargeBox(params.getChargeBoxId())) {
            String errMsg = String.format("Active task found on ChargeBox %s!",
                    params.getChargeBoxId()
        ...
        Integer newTransactionId = transactionRepository.getActiveTransactionId(params.getChargeBoxId(), params.getConnectorId());

        client.remoteStopTransaction(transactionParams, "SteveWebApi");
...
//        Transaction transaction = transactionRepository.getTransaction(newTransactionId);
        ...
        while (System.currentTimeMillis() - waitStart < TIMEOUT_MS) {
            transaction = transactionRepository.getTransaction(newTransactionId);
            if (transaction.getStopValue() != null) {
                break; // нашли запись — выходим
            }
            try {
                Thread.sleep(POLL_INTERVAL_MS);
                ...
                // Восстанавливаем статус прерывания и выходим — чтобы не игнорировать внешний interrupt
                Thread.currentThread().interrupt();
                throw new IllegalStateException("Thread interrupted while waiting for transaction set energy value", ie);
            }
        }

//        return new RemoteStopResponse(
//                0,
//                "CP_TEST",
//                0,
//                "CP_TEST_TAG",
//                "0",
//                "2025-08-26 15:53:08.000000"
//        );
